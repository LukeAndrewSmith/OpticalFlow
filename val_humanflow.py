...
import models
from multiscaleloss import motion_warping_error, realEPE
...
def main():
    ...
    for img_paths, flow_path, seg_path in tqdm(make_dataset(args.data)):
        ...
        raw_im2 = flow_transforms.ArrayToTensor()(
            255*imread(img_paths[1])[:, :, :3])
        ...
        gtflow_var = target_transform(
            load_flo(flow_path)).unsqueeze(0).to(device)
        segmask_var = flow_transforms.ArrayToTensor()(
            cv2.imread(seg_path)).unsqueeze(0).to(device)
