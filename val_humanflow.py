...
def main():
    ...
    for i, (img_paths, flow_path, seg_path) in enumerate(tqdm(test_list)):
        ...
        if args.output_dir is not None:
            ...
            # predicted flow file export
            flow_export_path = output_path.replace('/flow/','/')
            print(f"output path: {output_path}")
            flow_write(flow_export_path,  output.cpu()[0].data.numpy()[0],  output.cpu()[0].data.numpy()[1])
            
            # heatmap export
            hmap_path = output_path.replace('/flow/','/hmap/').replace('.flo', '.png')
            hmap_dir = os.path.dirname(hmap_path)
            if not os.path.exists(hmap_dir):
                print(f"exporting hmap: made directory {hmap_dir}")
                os.mkdir(hmap_dir)
            plt.imshow(mapepe, cmap='Reds', vmax=5, vmin=0)
    ...
    print("Average EPE",flow_epe.avg )
    print("Motion warping error", avg_mot_err.avg)


def realEPEMapGTDS(output, target):
    """ Resizes ground truth to output size, and then calculates EPE for each pixel,
    returning the EPE matrix with the value for each pixel.
    """
    b, _, h, w = output.size()
